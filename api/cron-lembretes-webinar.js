...
const JANELAS = {
  ...
  hora: { de: Date.parse('2026-10-07T19:00:00Z'), ate: Date.parse('2026-10-07T19:50:00Z'), email: true, sms: true },      // 7 out, 20h00
  abertura: { de: Date.parse('2026-10-07T19:53:00Z'), ate: Date.parse('2026-10-07T20:10:00Z'), email: false, sms: true }, // 7 out, 20h53
};
const LINK_CURTO = 'oferta.vloom.pt/webinar2026/entrar';
const SMS = {
  vespera: `Vloom: amanhã, quarta, às 21h00, é o webinar ao vivo com o Tiago Severino. Link da sala: ${LINK_CURTO}`,
  hora: `Vloom: o webinar começa daqui a uma hora, às 21h00. Entre por aqui: ${LINK_CURTO}`,
  abertura: `Vloom: a sala do webinar já está aberta e começamos às 21h00. Entre agora: ${LINK_CURTO}`,
};
...
function emailHtml(tipo, { primeiro, zoom }) {
  const vespera = tipo === 'vespera', hoje = tipo === 'hoje';
  const titulo = vespera ? `É amanhã${primeiro ? ', ' + primeiro : ''}.` : hoje ? `É hoje à noite${primeiro ? ', ' + primeiro : ''}.` : `Começamos daqui a uma hora${primeiro ? ', ' + primeiro : ''}.`;
  const abertura = vespera
    ...
    : hoje ? 'Hoje, <b style="color:#252158">às 21h00 de Lisboa</b>, fazemos o webinar ao vivo no Zoom. A sala abre às 20h50. Guarde este email para ter o link à mão.'
    : 'Às <b style="color:#252158">21h00 em ponto</b> abrimos o webinar. A sala abre às 20h50 e pode entrar já com o link abaixo.';
  const lista = (vespera || hoje) ? `
  <tr><td style="padding:22px 40px 4px">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f7f7fb;border-radius:10px"><tr><td style="padding:20px 24px">
      <p style="margin:0 0 12px;font-size:13px;letter-spacing:.08em;text-transform:uppercase;font-weight:700;color:#8d489b">${vespera ? 'Para amanhã' : 'Para hoje à noite'}</p>
...
<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:14px;overflow:hidden;font-family:Helvetica,Arial,sans-serif;color:#252158">
  ...
  <tr><td style="padding:22px 40px 8px">
    <p style="margin:0 0 14px;font-size:16px;color:#3d3f5c">${vespera ? 'Até amanhã,' : hoje ? 'Até logo,' : 'Até já,'}</p>
...
const ASSUNTO = { vespera: 'Amanhã às 21h00: o seu lugar no webinar', hoje: 'É hoje às 21h00: o link da sala', hora: 'Começamos daqui a uma hora (21h00)' };
...
async function correr({ agora = Date.now(), simular = false, teste = false } = {}) {
  ...
  const enviar = async ({ canal, c }) => {
    ...
    const corpo = canal === 'email'
      ? { type: 'Email', contactId: c.id, subject: ASSUNTO[tipo], html: emailHtml(tipo, { primeiro, zoom }) }
      : { type: 'SMS', contactId: c.id, message: SMS[tipo] };
    const r = await fetch(`${GHL}/conversations/messages`, { method: 'POST', headers, body: JSON.stringify(corpo) }).catch(() => null);
    return r ? r.status : 0;
  };
  ...
  if (faltam === 0 && !feitos.has(`${PREFIXO}resumo-${tipo}.json`)) {
    const nomes = { vespera: 'véspera', hoje: 'é hoje (12h)', hora: '1 hora antes', abertura: 'sala aberta (SMS)' };
    const nEmail = tarefas.filter(x => x.canal === 'email').length, nSms = tarefas.filter(x => x.canal === 'sms').length;
